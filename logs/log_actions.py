...
@tool
async def fetch_logs_for_dag(dag_id: str) -> dict:
    dag_runs_url = f"{AIRFLOW_URL}/api/v1/dags/{dag_id}/dagRuns"
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(dag_runs_url, auth=(USERNAME, PASSWORD))
            response.raise_for_status()
            ...
            for dag_run in dag_runs:
                ...
                task_response = await client.get(task_instances_url, auth=(USERNAME, PASSWORD))
                ...
                for task in task_instances:
                    ...
                    # Fetch logs
                    log_response = await client.get(logs_url, auth=(USERNAME, PASSWORD))
